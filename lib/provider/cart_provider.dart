...
class CartProvider extends ChangeNotifier {
  ...
  void setQuantity(bool isIncrement, int index) {
    if (isIncrement) {
      ...
      _isSelectedList[index] ? _amount = _amount + cartList[index].price : _amount = _amount;
    } else {
      _cartList[index].quantity = _cartList[index].quantity - 1;
      _isSelectedList[index] ? _amount = _amount - _cartList[index].price : _amount = _amount;
    }
  ...
  void toggleSelected(int index) {
    ...
    for (int i = 0; i < _isSelectedList.length; i++) {
      if (_isSelectedList[i]) {
        _amount = _amount + (_cartList[i].price * _cartList[index].quantity);
      }
    }
  ...
  void addToCart(CartModel cartModel) {
    ...
    _isSelectedList.add(true);
    cartRepo.addToCartList(_cartList);
    _amount = _amount + (cartModel.price * cartModel.quantity);
    notifyListeners();
  }
